...
目标：让下一会话少一次「重新发明」，而不是把手册写长。

本仓库自己也走 [loop](./dimensions/loop.md)：先让 `npm run verify` 红，再补文档变绿。七步执行看 [build-standard](https://github.com/miounet11/build-standard)。
...
6. 每个新的 block id 必须出现在 `examples/` 的 `` `ID` `` 里，或在 `gates.json` 标 `"evidence": "none"`（`SELF-7`）。
7. 跑 `npm run verify`。绿了才提。
...
- 不要删「禁止」段落。
- 案例可以过期；原则不为迁就某次实现变软。
- 不要把 ship-standard 的 `LAUNCH-*` / `RESIL-*` / `PROBE-*` / `DOC-*` 抄进现行清单。
...
- 可复现的攻击步骤或利用代码
- 标题或文件名声称「当前唯一版本」的文档（`SELF-4` 会失败）
