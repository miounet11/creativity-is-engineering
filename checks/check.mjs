#!/usr/bin/env node
// Self-check. This standard must pass its own SELF gates.
...
for (const f of mdFiles) {
  if (/最新|latest-version|FINAL/i.test(f)) {
    fail('SELF-5', `file name suggests a moving target: ${f}`);
  }
  ...
  for (const line of text.split('\n')) {
    if (/^#{1,6}\s.*最新/.test(line)) fail('SELF-5', `${f}: heading claims 最新 -> ${line.trim()}`);
  }
...
try {
  ...
  if (!/Generated by/.test(status)) fail('SELF-2', 'STATUS.md is missing the generated banner');
  if (!status.includes(catalog.version)) {
    fail('SELF-2', `STATUS.md does not show catalog version ${catalog.version} — run npm run status`);
  }
} catch {
  fail('SELF-2', 'STATUS.md is missing — run npm run status');
}
